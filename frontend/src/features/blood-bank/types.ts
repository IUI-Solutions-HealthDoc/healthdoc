...
export type BloodUnitStatus = "quarantined" | "available" | "reserved" | "issued" | "expired" | "discarded";
...
export interface BloodDonorCreate {
 full_name: string; age_years: number; sex: string; blood_group: string;
 mobile?: string | null; weight_kg: number; hemoglobin_g_dl: number;
 last_donation_date?: string | null; remarks?: string | null;
}
...
export interface BloodUnitCreate {
 bag_number: string; donor_id: string; blood_group: string; volume_ml: number;
 expiry_date: string; screening_status: ScreeningStatus;
}
