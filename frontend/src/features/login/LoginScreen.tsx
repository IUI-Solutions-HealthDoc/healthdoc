...
export function LoginScreen() {
  ...
  useEffect(() => {
    if (!isLoading && isAuthenticated && user?.role) {
      ...
      const target = safeReturnUrl(searchParams.get("redirect"), window.location.origin, fallback);
      window.location.replace(new URL(target).pathname === "/login" ? fallback : target);
    }
  }, [isAuthenticated, isLoading, user?.role, searchParams]);
  ...
  async function handleLogin() {
    ...
    setError(null);
    try {
      const target = safeReturnUrl(searchParams.get("redirect"), window.location.origin);
      await loginWithKeycloak(new URL(target).pathname === "/login" ? "/" : target);
    } catch {
      setError("Sign-in is unavailable. Please try again or contact your administrator.");
      setBusy(false);
    }
  }
    ...
    <div className="surface-card rounded-2xl border border-border/80 bg-card p-8 shadow-xl shadow-slate-900/5 transition-all">
      ...
      <div className="mt-6 space-y-4">
        ...
        <Button type="button" onClick={() => void handleLogin()}
          ...
          {busy ? "Redirecting…" : isLoading ? "Preparing sign-in…" : "Sign in with Keycloak"}
        </Button>
