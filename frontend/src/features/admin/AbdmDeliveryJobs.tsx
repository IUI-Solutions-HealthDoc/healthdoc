...
export function AbdmDeliveryJobs() {
  ...
  useEffect(() => {
    ...
    api<Job[]>(`/abdm/operations/jobs?status=${status}&limit=50&offset=${offset}`, { signal: controller.signal })
      .then((rows) => { if (!controller.signal.aborted) setJobs(rows); })
      .catch((reason) => { if (!controller.signal.aborted) { setJobs([]); setError(getUserFacingError(reason, "ABDM jobs could not be loaded.")); } });
  ...
  return <section className="surface-card space-y-4 p-5" aria-label="ABDM delivery jobs">
    ...
    {!jobs.length && <p>No jobs in this page.</p>}
