...
import type { ClinicalOrderSet } from "../types";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-2xl rounded-2xl border border-border bg-card p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        ...
        <p role="status" className="mt-4 text-sm">Preview only. No orders are placed here. Use the individual clinical order workflow until catalogue mappings are approved.</p>
                    ...
                    {selectedSet.orders.map((item, idx) => (
                      ...
                      >
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            <span className="font-semibold text-foreground block">{item.name || item.test_name || item.scan_name || item.directive}</span>
