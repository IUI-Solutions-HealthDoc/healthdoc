...
async def register_emergency_patient(
    ...
    facility_id is sourced from current_db_user (blocker 3 fix) — never
    from the request payload, so a nurse at facility A cannot register
    into facility B by supplying a different facility_id.
    """
...
async def approve_thid_promotion(
    ...
    """Different supervisor approves the promotion — generates UHID, updates patient."""
    facility = await db.get(Facility, current_db_user.facility_id)
    if not facility:
        raise HTTPException(404, "Facility not found")
    try:
