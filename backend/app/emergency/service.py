...
def _patient_snapshot(patient: Patient) -> dict:
    """Minimal snapshot for merge log before/after — mirrors patients/service.py."""
    return {
        "id": str(patient.id),
        "uhid": patient.uhid,
        "thid": patient.thid,
...
async def request_promotion(
    ...
    *,
    ...
    (schema doc v3.8, §Account governance).

    Does NOT generate the UHID yet — that happens only on approval, so a
    rejected promotion leaves the patient untouched.
    """
    ...
    # Check no pending promotion already exists for this patient
    existing = (
        await db.execute(
...
async def approve_promotion(
    ...
    *,
    ...
    """Supervisor (different from requester) approves the promotion.

    Generates a UHID and assigns it to the patient. Updates identity_path
    from 'thid' to 'demographics_only' — the patient is now a proper
    registered patient, identity still unverified until ABHA/Aadhaar flows.

    Locked with SELECT FOR UPDATE to prevent two concurrent approvals.
    Self-approval blocked (maker-checker, same rule as UHID merge).
    """
    merge_log = (
        await db.execute(
    ...
    uhid = await generate_uhid(db, state_code=state_code, facility_code=facility_code)
...
async def unmerge_promotion(
    ...
    *,
    ...
    """Supervisor (different from the approver) reverses an approved THID promotion.

    Schema doc v3.8: "unmerge is supervisor, never superadmin. The approving
    supervisor must be a different person from the one who approved the original
    merge." Router enforces supervisor role; this function enforces different-person.

    Restores patient to THID state: clears uhid, resets identity_path to 'thid'.
    The merge log status becomes 'unmerged' — the original approved log row is
    mutated, not a new row (the schema doc shows status as a mutable column here,
    unlike audit_logs which is append-only).
    """
    merge_log = (
        await db.execute(
            select(PatientMergeLog)
    ...
    if merge_log.source_type != "thid":
        raise ValueError("not_a_thid_promotion")
    # Different-approver rule (schema doc v3.8): the supervisor who approved
    # cannot also be the one who unmerges — that would let one person both
    # grant and revoke without any second set of eyes.
