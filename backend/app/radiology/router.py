...
async def schedule_radiology_order_item(
    item_id: uuid.UUID,
    ...
    if item.status != "placed":
        raise HTTPException(status_code=409, detail="Item is not in a schedulable state")
...
async def sign_off_radiology_report(
    ...
    tat_delta = new_report.created_at - item.created_at
