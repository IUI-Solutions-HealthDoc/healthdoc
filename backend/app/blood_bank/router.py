...
@router.get(
    ...
    dependencies=[Depends(require_roles("lab_tech", "pathologist", "nurse", "doctor", "admin", "superadmin"))],
)
...
@router.post(
    ...
    dependencies=[Depends(require_roles("lab_tech", "pathologist", "admin", "superadmin"))],
)
async def register_blood_donor(
    ...
    return await service.create_donor(db, payload, db_user.id)
...
@router.post(
    ...
    status_code=status.HTTP_201_CREATED,
    dependencies=[Depends(require_roles("lab_tech", "pathologist", "doctor", "admin", "superadmin"))],
)
async def register_blood_crossmatch(
    ...
    try:
        return await service.create_crossmatch(db, payload, db_user.id)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))
...
async def issue_blood_unit(
    ...
    try:
        return await service.issue_blood(db, payload, db_user.id)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))
