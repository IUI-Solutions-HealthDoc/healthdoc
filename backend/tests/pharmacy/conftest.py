...
async def pharmacy_seed(db_session: AsyncSession) -> dict[str, uuid.UUID]:
    ...
    await db_session.execute(text("""
        ...
        VALUES (:id, :encounter_id, :patient_id, :facility_id, :created_by)
    """), {"id": prescription_id, "encounter_id": encounter_id, "patient_id": patient_id,
             "facility_id": facility_id, "created_by": doctor_id})
    await db_session.execute(text("""
