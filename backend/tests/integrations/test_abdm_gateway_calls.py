...
class _StubClient:
    ...
    async def request(self, method, path, *, json=None, extra_headers=None, request_id=None):
        self.calls.append({
            ...
            "headers": dict(extra_headers or {}), "request_id": request_id,
        })
...
async def test_an_hi_request_refuses_an_unset_push_url(monkeypatch, stub):
    ...
    with pytest.raises(hiu_gw.DataPushUrlNotConfigured):
        await hiu_gw.request_health_information(
            consent_id="c-1", date_from=FROM, date_to=TO,
            dh_public_key="k", key_expiry=TO, nonce="n",
        )
    assert stub.calls == []
...
async def test_link_care_contexts_sends_the_link_token_header(stub):
    ...
    await hip_gw.link_care_contexts(
        abha_address="ram@sbx", link_token="LT-1", display="Blood Test",
...
async def test_notify_care_context_shape(stub):
    await hip_gw.notify_care_context(
        abha_address="ram@sbx", care_context_reference="V-1",
...
async def test_on_discover_echoes_the_gateways_request_id(stub):
    ...
    await hip_gw.respond_to_discovery(
        transaction_id="T-1", gateway_request_id="GW-REQ-9", abha_address="ram@sbx",
        display="OPD", care_contexts=[{"referenceNumber": "V-1", "display": "OPD"}],
        hi_type="OPConsultation", matched_by=["MR"],
    )
    call = stub.last
...
async def test_on_discover_with_no_matches_sends_an_empty_patient_list(stub):
    ...
    await hip_gw.respond_to_discovery(
        transaction_id="T-1", gateway_request_id="GW-1", abha_address="ram@sbx",
        display="OPD", care_contexts=[], hi_type="OPConsultation", matched_by=[],
    )
...
async def test_hi_transfer_notification_identifies_us_as_the_hip(stub):
    await hip_gw.notify_hi_transfer(
        consent_id="C-1", transaction_id="T-1", session_status="TRANSFERRED",
...
async def test_consent_request_shape(stub):
    await hiu_gw.request_consent(
        abha_address="ram@sbx", hi_types=["Prescription"],
        date_from=FROM, date_to=TO, expiry=TO + timedelta(days=30),
    )
    call = stub.last
...
async def test_health_information_request_carries_key_material_and_push_url(stub):
    await hiu_gw.request_health_information(
        consent_id="C-1", date_from=FROM, date_to=TO,
        dh_public_key="PUBKEY", key_expiry=TO, nonce="NONCE-1",
    )
    call = stub.last
    ...
    assert hi["dataPushUrl"] == (
        "https://abdm.example.org/api/v1/abdm/hiu/callbacks/health-information/transfer"
    )
    km = hi["keyMaterial"]
...
async def test_hiu_receipt_notification_identifies_us_as_the_hiu(stub):
    ...
    await hiu_gw.notify_hi_receipt(
        consent_id="C-1", transaction_id="T-1", session_status="RECEIVED",
        hip_id="OTHER_HIP",
...
async def test_a_backwards_date_range_is_refused(stub):
    with pytest.raises(ValueError, match="before"):
        await hiu_gw.request_consent(
            abha_address="ram@sbx", hi_types=["Prescription"],
            date_from=TO, date_to=FROM, expiry=TO,
        )
    assert stub.calls == []
...
async def test_timestamps_use_a_literal_z_not_an_offset(stub):
    ...
    await hiu_gw.request_consent(
        abha_address="ram@sbx", hi_types=["Prescription"],
        date_from=FROM, date_to=TO, expiry=TO,
    )
...
async def test_a_naive_datetime_is_treated_as_utc_not_local(stub):
    ...
    await hiu_gw.request_consent(
        abha_address="ram@sbx", hi_types=["Prescription"],
        date_from=datetime(2026, 1, 1), date_to=datetime(2026, 6, 1),
