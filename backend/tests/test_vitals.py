...
async def seeded_encounter_and_admission(db_session, fake_facility):
    ...
    visit = Visit(
        ...
        visit_type="opd",
        status="registered",
        visit_date="2026-07-28T09:00:00+00:00",
    ...
    admission = Admission(
        ...
        ward_id=ward.id,
        bed_id=bed.id,
        admitted_at="2026-07-28T09:00:00+00:00",
        status="admitted",
        created_by=doctor.id,
    )
...
async def test_create_vitals_with_encounter_succeeds(authed_client: AsyncClient, seeded_encounter_and_admission):
    ...
    response = await authed_client.post(f"/encounters/{encounter.id}/vitals", json=payload)
...
async def test_list_vitals_for_encounter(authed_client: AsyncClient, seeded_encounter_and_admission):
    ...
    await authed_client.post(f"/encounters/{encounter.id}/vitals", json=payload)

    response = await authed_client.get(f"/encounters/{encounter.id}/vitals")
